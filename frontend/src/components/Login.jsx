...
import { BUTTON_CLASSES, BUTTONCLASSES, INPUTWRAPPER } from "../assets/dummy";
import { useNavigate } from "react-router-dom";
import axios from "axios";

const INITIAL_FORM = { email: "", password: "" };
export default function Login({ onSubmit, onSwitchMode }) {
  ...
  return (
    <div className="max-w-md bg-white shadow-lg border border-purple-100 rounded-xl p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {fields.map(({ name, type, placeholder, icon: Icon, isPassword }) => (
          <div key={name} className={INPUTWRAPPER}>
            <Icon className="text-purple-500 w-5 h-5 mr-3" />
            <input
              type={type}
